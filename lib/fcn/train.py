...
class SolverWrapper(object):
    ...
    def train_model_vertex_pose(self, sess, train_op, loss, loss_cls, loss_vertex, loss_pose, loss_regu, learning_rate, max_iters, data_layer):
        """Network training loop."""
        # add summary
        # tf.summary.scalar('loss', loss)
        # merged = tf.summary.merge_all()
        ...
        coord = tf.train.Coordinator()
        if cfg.TRAIN.VISUALIZE or 1:
            load_and_enqueue(sess, self.net, data_layer, coord, 0)
        else:
            print("threading load_and_enqueue.")
            t = threading.Thread(target=load_and_enqueue, args=(sess, self.net, data_layer, coord))
            t.start()
            print("load_and_enqueue finishes.")
        # print 'Solving...1_2'
        ...
        for iter in range(max_iters):
            ...
            coord = tf.train.Coordinator()
            load_and_enqueue(sess, self.net, data_layer, coord, iter)

            timer.tic()
            loss_value, loss_cls_value, loss_vertex_value, loss_pose_value, loss_regu_value, lr, label_2d_val, poses_pred_val, poses_tahn_v, poses_w_v, h0, h1, h2, h3, _ = sess.run([loss, loss_cls, loss_vertex, loss_pose, loss_regu, learning_rate, \
            self.net.get_output('label_2d'),
            self.net.get_output('poses_pred'),
            self.net.get_output('poses_tanh'),
            self.net.get_output('poses_weight'),
            self.net.get_output('hough')[0],
            self.net.get_output('hough')[1],
            self.net.get_output('hough')[2],
            self.net.get_output('hough')[3],                
             train_op],
              options=run_options, run_metadata=run_metadata)
            # loss_value, loss_cls_value, loss_vertex_value, loss_pose_value, loss_regu_value, lr, label_2d, h0, h1, h2, h3, _ = sess.run([
            ...
            print('Solving... all losses and train_op')           

            print(">>>> This is the label_2d: ", np.sum(label_2d_val) )
            print(">>>> This is the poses_pred: ", (poses_pred_val.shape) )
            print(">>>> This is the poses_tanh: ", (poses_tahn_v.shape) )
            print(">>>> This is the poses_wei: ", (poses_w_v.shape) )
            # print(">>>> This is the prob_norm: ", np.sum(prob_norm_val) )
            # print(">>>> This is the prob_norm shape: ", prob_norm_val.shape )
            # print(">>>> This is the prob_norm 0C: :", prob_norm_val[0,240,320,:] )
            # print(">>>> This is the prob_norm 1C: :", prob_norm_val[1,240,320,:] )            
            print(">>>> This is the h0: ", np.sum(h0) )
            print(">>>> This is the h1: ", np.sum(h1) )
            print(">>>> This is the h2: ", np.sum(h2) )
            print(">>>> This is the h3: ", np.sum(h3) )

            # train_writer.add_summary(summary, iter)
            timer.toc()
            
            print 'iter: %d / %d, loss: %.4f, loss_cls: %.4f, loss_vertex: %.4f, loss_pose: %.4f, loss_regu: %.4f, lr: %.8f,  time: %.2f' %\
                    (iter+1, max_iters, loss_value, loss_cls_value, loss_vertex_value, loss_pose_value, loss_regu_value, lr, timer.diff)
...
def load_and_enqueue(sess, net, data_layer, coord, iter):
    ...
    while not coord.should_stop():
        ...
        print('[load_and_enqueue] get data_blob')
        if cfg.INPUT == 'RGBD':
        ...
        buf.write("data_blob_%s.png" % (iter) )
        cv2.imwrite(buf.getvalue(), data_blob[0,:,:,:])
        print('%s has been written.' % (buf.getvalue()))

        print('[load_and_enqueue] collect feed_dict')
        if cfg.TRAIN.SINGLE_FRAME:
            if cfg.TRAIN.SEGMENTATION:
                if cfg.INPUT == 'RGBD':
                    if cfg.TRAIN.VERTEX_REG_2D or cfg.TRAIN.VERTEX_REG_3D:
                        print('SINGLE_FRAME & SEGMENTATION & INPUT == RGBD & cfg.TRAIN.VERTEX_REG_2D or cfg.TRAIN.VERTEX_REG_3D')
                        feed_dict={net.data: data_blob, net.data_p: data_p_blob, net.gt_label_2d: blobs['data_label'], net.keep_prob: 0.5, \
                                   ...
                                   net.poses: blobs['data_pose'], net.extents: blobs['data_extents'], net.meta_data: blobs['data_meta_data'], net.points: blobs['data_points'], net.symmetry: blobs['data_symmetry']}
                    else:
                        print('SINGLE_FRAME & SEGMENTATION & INPUT == RGBD & !(cfg.TRAIN.VERTEX_REG_2D or cfg.TRAIN.VERTEX_REG_3D)')
                        feed_dict={net.data: data_blob, net.data_p: data_p_blob, net.gt_label_2d: blobs['data_label'], net.keep_prob: 0.5}
                else:
                ...
                if cfg.INPUT == 'RGBD':
                    print('SINGLE_FRAME & !SEGMENTATION & INPUT = RGBD')
        ...
        # to finish one loop of data queue and return
        coord.request_stop()
        print("finished one loop of data queue. coord.request_stop().")
        ...
        if cfg.TRAIN.VERTEX_REG_2D or cfg.TRAIN.VERTEX_REG_3D:
            if cfg.TRAIN.POSE_REG:
                if cfg.TRAIN.ADAPT:
                    ...
                    sw.train_model_vertex_pose(sess, train_op, loss, loss_cls, loss_vertex, loss_pose, loss_regu, learning_rate, max_iters, data_layer)
            else:

...
def im_segment_single_frame(sess, net, im, im_depth, meta_data, voxelizer, extents, points, symmetry, num_classes):
    """segment image
    """
    print("[im_segment_single_frame] starts")
    ...
    print("[test.py][im_segment_single_frame] enqueue finishes.")
    ...
    if cfg.NETWORK == 'FCN8VGG':
        ...
        if cfg.TEST.VERTEX_REG_2D:
            if cfg.TEST.POSE_REG:
                ...
                print ("vtx_pred sum = {}", np.sum(vertex_pred))
                cv2.imwrite("cls1_vx.png", vertex_pred[0,:,:,3])
                cv2.imwrite("cls1_vy.png", vertex_pred[0,:,:,4])
                cv2.imwrite("cls1_Z.png", vertex_pred[0,:,:,5])
                cv2.imwrite("cls2_vx.png", vertex_pred[0,:,:,6])
                cv2.imwrite("cls2_vy.png", vertex_pred[0,:,:,7])
                cv2.imwrite("cls2_Z.png", vertex_pred[0,:,:,8])
...
def test_net_single_frame(sess, net, imdb, weights_filename, model_filename):
    ...
    for i in perm:
        ...
        if cfg.TEST.SYNTHETIC:
            ...
            print("[test.py] !cfg.TEST.SYNTHETIC")
            ...
            # load meta data
            print("metadata_path", imdb.metadata_path_at(i))
            meta_data = scipy.io.loadmat(imdb.metadata_path_at(i))
...
def compute_translations(dets, poses, points, intrinsic_matrix):
    ...
    print("[test.py] compute_translations")
...
def im_detect_single_frame(sess, net, im, im_depth, meta_data, points, symmetry, num_classes):
    ...
    print("[test.py][im_detect_single_frame] before calling sess.run")

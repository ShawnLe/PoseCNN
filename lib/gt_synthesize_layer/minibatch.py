...
def _get_label_blob(roidb, intrinsic_matrix, data_out, num_classes, db_inds_syn, im_scales, extents, \
    ...
    for i in xrange(num_images):
        ...
        if is_adapt:
            print("[_get_label_blob] is_adapt")
            filename = cfg.TRAIN.ADAPT_ROOT + '{:06d}-depth.png'.format(db_inds_adapt[i])
            ...
            if is_syn:
                # print("[_get_label_blob] !is_adapt & is_syn")
                if cfg.TRAIN.SYN_ONLINE:
                    print("[_get_label_blob] !is_adapt & is_syn & cfg.TRAIN.SYN_ONLINE")
    ...
    print("sum label_blob = {}", np.sum(label_blob))

...
class Synthesizer
{
 public:
  ...
  // <!-- sonle
  Synthesizer(std::string model_file, std::string pose_file, int project_keypoint_, int kpt_num_, std::string root_dir_);

  void selectModelPoints();

  int vertice_is_good(int class_num, int sel_num, unsigned int vertice_idx);
  // -->
 ...
 private:
  ...
  // <!--sonle
  int project_keypoint;
  int kpt_num;
  std::string root_dir;
  int width_, height_;

  std::vector<Vec3> selMdlPoints;
  // -->
